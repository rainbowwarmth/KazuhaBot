...
class BrowserManager {
  ...
  async renderinit() {
    try {
      const targetDir = `${_path}/node_modules/puppeteer/.local-chromium`;
      const dirExists = fs.existsSync(targetDir);
      const isEmpty = dirExists && fs.readdirSync(targetDir).length === 0;

      if (!dirExists || isEmpty) {
        logger.mark("未安装浏览器，开启安装Puppeteer")
